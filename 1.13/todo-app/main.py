...
def render_todos():
    items = "".join(f"<li>{todo}</li>" for todo in HARDCODED_TODOS)
    return f"<ul id='todo-list'>{items}</ul>"
  ...
  <script>
    ...
    input.addEventListener('input', () => {{
      if (input.value.length >= 140) {{
        warning.textContent = 'Max 140 characters reached';
      }} else {{
        warning.textContent = '';
      }}
    }});
    ...
    form.addEventListener('submit', (e) => {{
      e.preventDefault();
      // sending todos to the backend comes in a future exercise
    }});
  </script>
</body>
</html>
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == "/":
            ...
            self.end_headers()
            html = HTML_TEMPLATE.format(todo_list=render_todos())
